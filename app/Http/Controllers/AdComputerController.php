...
class AdComputerController extends Controller
{
        ...
        try {
            ...
            if (!$process->isSuccessful()) {
                ...
                $this->logAdActivity(
                    ...
                    targetUserName: null,
                    success: false,
                    errorMessage: 'Erreur PowerShell : ' . $process->getErrorOutput()
                );
    ...
    public function getAllLapsComputers(Request $request)
    {
        ...
        // Script PowerShell qui récupère tous les ordinateurs
        $psScript = <<<'PS'
try {
    ...
    $result = foreach ($c in $computers) {
        ...
        try {
            ...
            # on ignore l'erreur, laisse $pwd = $null
        }
...
PS;
        ...
        try {
            ...
            $computerNames = array_filter(array_column($computersFormatted, 'name'));
            $computerNamesList = implode(', ', array_slice($computerNames, 0, 10));
            if (count($computerNames) > 10) {
                $computerNamesList .= ' (+' . (count($computerNames) - 10) . ' autres)';
            }

            $this->logAdActivity(
                action: 'get_all_laps_computers',
                targetUser: null,
                targetUserName: $computerNamesList,
                success: true,
                additionalDetails: [
                    'total_computers' => count($computersFormatted),
                    'method' => 'PowerShell LAPS Bulk Retrieval',
                    'timestamp' => now()->toDateTimeString()
                ]
            );
