...
public function findUser(Request $request)
{
    ...
    try {
        ...
        if (empty($output) || $output === 'null') {
            ...
            // ✅ Logger la recherche sans résultats
            $this->logAdActivity(
                action: 'search_user',
                targetUser: $search,
                targetUserName: null,
        ...
        if (!$adUsers || json_last_error() !== JSON_ERROR_NONE || empty($adUsers)) {
            ...
            $this->logAdActivity(
                ...
                targetUserName: null,
                success: false,
                errorMessage: 'Données AD invalides : ' . json_last_error_msg()
            );
        ...
        // ✅ Logger la recherche avec les résultats trouvés
        $foundNames = $authorizedUsers->pluck('name')->toArray();
        
        $this->logAdActivity(
            action: 'search_user',
            targetUser: $search,
            targetUserName: !empty($foundNames) ? implode(', ', $foundNames) : null, // ✅ Ajouter les noms trouvés
            success: true,
            additionalDetails: [
                'search_query' => $search,
